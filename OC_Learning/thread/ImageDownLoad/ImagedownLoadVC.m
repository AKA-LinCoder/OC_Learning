...
{
    NSArray *arm = self.apps;
    //检查缓存数据
    for (NSInteger i = 0; i<arm.count; i++) {
        LinAPP *appLin = arm[i];
        UIImage *image = [self.images objectForKey:appLin.icon];
        if (image) {
            NSLog(@"存在缓存图片%ld",(long)i);
        }
    }
}
...
//    [cell.imageView sd_setImageWithURL:app.icon placeholderImage:[UIImage imageNamed:@"people"] options:0 context:nil  progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL *  targetURL) {
//        NSLog(@"%f",1.0*receivedSize/expectedSize);
//    } completed:^(UIImage *  image, NSError *  error, SDImageCacheType cacheType, NSURL *  imageURL) {
//        NSLog(@"使用的是%zd",cacheType);
//
//    }];
//    return cell;
//}
#pragma mark - 自定义实现突破下载内存缓存磁盘缓存
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if(cell == nil){

...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reciveNotic) name:@"note" object:nil];
    //方式二，因为没有观察者
    observe =  [[NSNotificationCenter defaultCenter] addObserverForName:@"note" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        //只要监听到通知就会调用
